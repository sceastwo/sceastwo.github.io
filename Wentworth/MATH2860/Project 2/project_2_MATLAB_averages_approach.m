...
%%%%%%%%%%%%%%%%% DEBUG %%%%%%%%%%%
%%%%%%%%%%%%% Solving the system via iteration %%%%%%%%%%%%%%%%%%%
debug_flag = 0;
...
if debug_flag
    ...
    daspect([1 1 1]);
    hold off;
    while (isvalid(g_nodes_ex(1).point))
        new_node_displacements_vector = zeros(6*num_of_nodes, 1);
        ...
        for j = 1:num_of_nodes
            ...
            Q_total = zeros(3,3);
            q_sum = -forces_vector(i_q);
            W_total = zeros(3,3);
            w_sum = -forces_vector(i_w);

            for j2 = 1:num_of_beams
                skip_flag = 0;
                if (beams_ex(j2).n1_float_flag && beams_ex(j2).n1_index == j)
                    if (beams_ex(j2).n2_float_flag && beams_ex(j2).n2_index == j)
                        skip_flag = 1;
                        continue;
                    else 
                        skip_flag = 0;

                        if (beams_ex(j2).n2_float_flag)
                            i_q_far = (6*beams_ex(j2).n2_index - 5):(6*beams_ex(j2).n2_index - 3);
                            i_w_far = (6*beams_ex(j2).n2_index - 2):(6*beams_ex(j2).n2_index);
                            q_far = node_displacements_vector(i_q_far);
                            w_far = node_displacements_vector(i_w_far);
                        else
                            q_far = zeros(3,1);
                            w_far = zeros(3,1);
                        end
                        ...
                        L = beams_ex(j2).L;
                        t = beams(j2).t;
                        proj = beams_ex(j2).proj;
                        perp = beams_ex(j2).perp;
                        rot = beams_ex(j2).rot;

                        Q = (t/L)*(k_par*proj + k_perp*perp); 
                        W = (t/L)*(ka_par*proj + ka_perp*perp);
                    end
                elseif (beams_ex(j2).n2_float_flag && beams_ex(j2).n2_index == j)
                    skip_flag = 0;

                    if (beams_ex(j2).n1_float_flag)
                        i_q_far = (6*beams_ex(j2).n1_index - 5):(6*beams_ex(j2).n1_index - 3);
                        i_w_far = (6*beams_ex(j2).n1_index - 2):(6*beams_ex(j2).n1_index);
                        q_far = node_displacements_vector(i_q_far);
                        w_far = node_displacements_vector(i_w_far);
                    else
                        q_far = zeros(3,1);
                ...
                if (~skip_flag)
                    ...
                    q_sum = q_sum + Q*(q_far + L*rot*(node_displacements_vector(i_w) + w_far)/2);
                    w_sum = w_sum + W*(w_far - gamma*rot*(node_displacements_vector(i_q) - q_far ...
                            + L*rot*(node_displacements_vector(i_w) + w_far)/2));
                end
            end

            %%%% Update displacements and torsion.
            new_node_displacements_vector(i_q) = Q_total\q_sum;
            new_node_displacements_vector(i_w) = W_total\w_sum;
        end

        node_displacements_vector = new_node_displacements_vector;
