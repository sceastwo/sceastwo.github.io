...
%%%%%%% The script that contains your input data is invoked here.
design_1
%%%%%%%
...
if gravity_flag
    ...
    gravity = 1;
    for i = 1:num_of_beams
        mass = density * beams(i).t * beams_ex(i).L;
        weight = [0; 0; -gravity * mass];
        if (beams_ex(i).n1_float_flag)
            i_n1_q = (6*beams_ex(i).n1_index - 5):(6*beams_ex(i).n1_index - 3);
            i_n1_w = (6*beams_ex(i).n1_index - 2):(6*beams_ex(i).n1_index);
            
            forces_vector(i_n1_q) = forces_vector(i_n1_q) - weight / 2;
        end
        if (beams_ex(i).n2_float_flag)
            i_n2_q = (6*beams_ex(i).n2_index - 5):(6*beams_ex(i).n2_index - 3);
            i_n2_w = (6*beams_ex(i).n2_index - 2):(6*beams_ex(i).n2_index);
            
            forces_vector(i_n2_q) = forces_vector(i_n2_q) - weight / 2;
        end
    end
end
%%%%%%%%%%%%%%%%%%% END gravity setup
...
length_limit = 3.0;
for i = 1:num_of_beams
    if (beams_ex(i).L > length_limit)
        error('Error: beam #%i exceeds the length limit with a length of %f', i, beams_ex(i).L);
    end
end
%%%%%%%%%%%%%%%%%%% END beam length check
